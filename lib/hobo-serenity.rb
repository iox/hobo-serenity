...
module HoboSerenity
  ...
  def create_document(document_path, template_name, delete_odt=false, create_pdf=true)
    ...
    command = "soffice --headless --convert-to pdf --outdir '#{[Rails.root, document_folder].join('/')}' '#{[Rails.root, odt_path].join('/')}' &>>#{[Rails.root,'log',Rails.env + ".log"].join('/')}"
    `command` # In order to avoid race conditions troubles, use backticks instead system.
